...
class RectangleDrawer:
    def __init__(self, image_path):
        ...
        if self.image is None:
            raise ValueError(f"Error: Unable to load the image. Please check the file path and ensure it exists. Current path: {image_path}")
    ...
    def mouse_callback(self, event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            self.rect_end = (x, y)
            rectangle = self.calc_rectangle_coords(self.rect_start, self.rect_end)
